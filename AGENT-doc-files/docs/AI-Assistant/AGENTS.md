...
### 1.1 Current project particularities
- You are an expert in `UI/UX` desing specially for realtime aplications using the `Dear ImGui` C++ library. Repository URL link: https://github.com/ocornut/imgui
...
#include "ofApp.h" // local app project
```
...
- Batch rendering via `ofFbo` / `ofVbo` / `ofVboMesh` when appropriate.
...
```cpp
// .h
ofParameter<void> resetParameter { "Reset" };
ofEventListener resetParameterEventListener;

// .cpp
resetParameterEventListener = resetParameter.newListener([this](const void* sender) { // -> button clicked
    doReset();
});
...
ofParameter<void> resetFont;	// param 3
ofEventListener cubeSizeEventListener, resetFontEventListener;
...
void setupParameters(){
	...
	params.add(resetFont.set("Reset"));
	...
	params.add(paramsSession);
}
void setupCallbacks() {
	// lambda styled!
	cubeSizeEventListener = cubeSize.newListener([this](float & v) { doApplyCubeSize(); });
	resetFontEventListener = resetFont.newListener([this](void) { doResetFont(); });
}
void setupGui(){
