//
//  TonemapCrossfadeShader.h
//  ofxMarkSynth
//
//  Two-input HDR crossfade + tonemapping shader.
//
...
class TonemapCrossfadeShader : public ::Shader {

public:
  void begin(int tonemapType,
             float exposure,
             float gamma,
             float whitePoint,
             float contrast,
    ...
    shader.setUniformTexture("u_textureA", textureA, 0);
    shader.setUniformTexture("u_textureB", textureB, 1);
  }
  ...
  std::string getFragmentShader() override {
    return GLSL(
      ...
      vec3 sampleTexture(sampler2D tex, vec2 uv, int flipY) {
        vec2 sampleUv = uv;
        if (flipY == 1) {
