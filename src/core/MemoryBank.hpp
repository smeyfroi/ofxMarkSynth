...
public:
    ...
    /// Save a specific crop (top-left origin) to a slot.
    /// Origin is clamped so the crop fits inside the source.
    void saveToSlotCrop(const ofFbo& source, int slot, glm::vec2 cropTopLeft);
    ...
    /// @return slot index saved, or -1 if none
    int processPendingSave(const ofFbo& source);
...
private:
    ...
    /// Capture a crop from source into the destination FBO
    void captureCrop(ofFbo& dest, const ofFbo& source, glm::vec2 cropTopLeft);
